...
def loading_data(exp_specs, batch_size: int, num_workers: int = 0) -> DataLoader:
    ...
    train_dataset = dataset(
        exp_specs=exp_specs, root=exp_specs['root'], img_dim=exp_specs['img_dim'], mri_type=exp_specs['mri_type'], train=True,
    )
    val_dataset = dataset(
        exp_specs=exp_specs, root=exp_specs['root'], img_dim=exp_specs['img_dim'], mri_type=exp_specs['mri_type'], train=False,
    )
